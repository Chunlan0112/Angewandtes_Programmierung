...
with open(filename, 'r') as myfile:
    for line in myfile:
        try:
            word, tag, lemma = line.split()  # besser: word, tag, lemma = line.split("\t")
        except ValueError:  # some lines have more than three element
            pass
        if tag == 'NN':  # besser:  if tag == 'NN' and word.isalnum() and len(word) >= 3:
...
for word in no_repete_word:
    ...
    for ele in compound(word, freq_dict):
        grade = 1
        for el in ele:
            grade *= freq_dict[el]  # oder: grade = math.prod(freq_dict[el] for el in ele)
